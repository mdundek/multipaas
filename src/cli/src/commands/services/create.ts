...
export default class CreateService extends Command {
	...
	async run() {
		...
		choices.sort((a,b) => (a.name > b.name) ? 1 : ((b.name > a.name) ? -1 : 0));
		...
		if(serviceVersionCfg.provision_volume) {
			...
			if(!this.validNameRegEx.test(sName)){
				...
				for(let param in serviceParams){
					...
					if(!customConfigFile.response && paramCfg.prompt){
						...
						while(true) {
							...
							if(paramCfg.regexValidate){
								let regexp = new RegExp(paramCfg.regexValidate);
								if(!regexp.test(paramValue)) {
									this.logWarn(paramCfg.errorMsg);
								} else {
