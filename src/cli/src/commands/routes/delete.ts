...
export default class Route extends Command {
	static description = 'delete a route from this workspace'
	 ...
	 */
	async run() {
		let result = await this.api("routes", {
			method: "get-routes",
			data: {}
		});
		if(this.handleError(result)){
			if(result.data.length == 0){
				return this.logError("no routes found");
			}

			// Select target domain
			let routeChoice: any = await inquirer.prompt([{
				name: 'route',
				message: 'What route do you want to delete:',
				type: 'list',
			...
			result = await this.api("routes", {
				method: "delete",
				data: {
					"routeId": routeChoice.route
				}
			});
		
			if(this.handleError(result)){
				this.log("Route deleted successfully");
			}
		}
	}
	...
	handleError(result: { code: number }) {
		if(result.code == 401){
			...
			this.logError(`You do not have sufficient permissions to delete routes`);
			return false;
		} else if(result.code == 404){
			this.logError(`This route does not exist`);
			return false;
		} else if(result.code == 409){
			this.logError(`This route is in use, therefore it cant be deleted`);
			return false;
