...
class MqttController {
    ...
    static init() {
        ...
        this.client.on('connect', () => {
            this.online = true;
            this.client.subscribe(`/multipaas/alert/#`);
            this.client.subscribe(`/multipaas/task/new/#`);
            this.client.subscribe(`/multipaas/k8s/host/respond/taskmanager/#`);
            this.client.subscribe(`/multipaas/k8s/host/query/taskmanager/#`);
    ...
    static async _processIncommingMessage(topic, message) {
        ...
        if(!this.expected(topicSplit, message)){
            ...
            if(topic.startsWith("/multipaas/alert/")){
                console.error("ALERT =>", topic);
            } else if(topic.startsWith("/multipaas/task/new/")){
                let taskId = topicSplit[3];
                if(taskId.length > 0 && !isNaN(taskId)){
                    TaskController.processPendingTasks(taskId);
                }
            } 
            else if(topic.startsWith("/multipaas/k8s/host/query/taskmanager/")){
                if(topicSplit[5] == "leaseIp"){
                    let freeIp = DHCPController.leaseFreeIp();
                    let payload = JSON.parse(message.toString());
                    this.client.publish(`/multipaas/k8s/host/respond/${payload.queryTarget}/${topicSplit[5]}/${topicSplit[6]}`, JSON.stringify({
                        "leasedIp": freeIp
                    }));
