...
if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 1 ]; then
        ...
        vagrant ssh -c "kubectl create secret docker-registry regcred --docker-server=registry.multipaas.org --docker-username=$3 --docker-password=$4 --docker-email=multipaas@multipaas.com" 2>/dev/null
        if [ $? -eq 1 ]; then
