...
if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            vagrant ssh -c "sudo bash /config_kublet_ip.sh" 2>/dev/null
            # vagrant ssh -c "kubectl create secret docker-registry regcred --docker-server=registry.multipaas.org --docker-username=$4 --docker-password=$5 --docker-email=multipaas@multipaas.com"
            # if [ $? -eq 1 ]; then
