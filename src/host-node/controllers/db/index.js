...
class DBController {
    ...
    /**
     * getApplicationsForWs
     * @param {*} wsId 
     * @param {*} ns 
     */
    static async getApplicationsForWs(wsId, ns, id) {
        let client = await this.pool.connect();
        let hasId = id != undefined && id != null;
        try {
            let query = `SELECT 
                    applications."id",
                    applications."name",
            ...
            if(hasId) {
                query += ` AND applications."id" = $2`;
            }
            if(ns) {
                query += ` AND applications."namespace" = ` + hasId ? "$3":"$2";
            }
            const res = await client.query(query, ns ? (hasId ? [wsId, id, ns] : [wsId, ns]) : (hasId ? [wsId, id] : [wsId]));
            return res.rows;
        } finally {
            client.release();
        }
    ...
    /**
     * getApplicationRoutesForWs
     * @param {*} wsId 
     * @param {*} ns 
     */
    static async getApplicationRoutesForWs(wsId, ns, id) {
        let client = await this.pool.connect();
        let hasId = id != undefined && id != null;
        try {
            let query = `SELECT 
     ...
     * @param {*} ns 
     */
    static async getApplicationVersionsForWs(wsId, ns, id) {
        let client = await this.pool.connect();
        let hasId = id != undefined && id != null;
        try {
            let query = `SELECT 
        ...
        try {
            const res = await client.query(`SELECT routes.* FROM 
                ...
                WHERE 
                    ...
                    (routes."serviceId" = services."id" AND services."workspaceId" = $2)`, [workspaceId, workspaceId]);
            return res.rows;
        } finally {
            client.release();
        }
