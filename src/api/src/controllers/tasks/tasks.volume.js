...
class TaskVolumeController {
    ...
    static async getWorkspacesVolumes(workspaceId, params) {
        ...
        let applicationVersions = await this.app.service("application-version").find({
            "query": {
                "volumeId": {
                    $in: volumes.data.map(o => o.id)
                }
        ...
        let applications = await this.app.service("applications").find({
            "query": {
                "id": {
                    $in: [...new Set(applicationVersions.data.map(o => o.applicationId))]
                }
            },
            "user": params.user,
            "authentication": params.authentication
        ...
        applicationVersions.data = applicationVersions.data.map(o => {
            let extra = applications.data.find(e => e.id == o.applicationId);
            o.serviceName = extra.name;
            o.namespace = extra.namespace;
            o.workspaceId = extra.workspaceId;
            return o;
        });
                ...
                bindings: volumeBindings.data.filter(o => o.volumeId == v.id).map(o => {
                    if(o.target == "workspace"){
                        return {
                            ...
                            "applications": applicationVersions.data.filter(a => a.volumeId == v.id)
                        };
                    } else {
