...
class TaskPvcController {
    ...
    static async listPVCs(data, params) {    
        ...
        if(response.data.status == 200){
            for(let i=0; i<response.data.output.pvc.length; i++) {
                let volumePathResponse = await MQTTController.queryRequestResponse(r.data[0].k8s_host.ip, "get_k8s_resource_values", {
                    "target": "pv",
                    "targetName": response.data.output.pvc[i].VOLUME,
                    "ns": data.ns,
                    "jsonpath": "{@.spec.local.path}",
                    "node": r.data[0]
                }, 15000);
                response.data.output.pvc[i].MOUNT_PATH = volumePathResponse.data.output[0];
            }
            return {
                "code": 200,
